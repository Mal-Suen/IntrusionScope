...
│   │   ├── sigma.go                 # Sigma 规则引擎
│   │   ├── yara.go                  # YARA 扫描器
│   │   ├── precise_match.go         # 精确匹配(NoCGO)
│   │   ├── rules_loader.go          # 规则加载器
│   │   ├── rust_engine.go           # Rust FFI (CGO)
│   │   └── rust_engine_nocgo.go     # NoCGO 降级实现
│   └── signature/                   # 特征库管理
│       ├── signature.go
│       └── parser.go
├── engine/                         # Rust 高性能检测引擎
...
├── DESIGN.md
├── REQUIREMENTS.md
└── intrusionscope.exe              # 编译产物(Windows)
```
...
**创建日期**: 2026-04-15
**最后更新**: 2026-05-21
**状态**: 完善版 - 修复项目结构与实际代码一致；补充 FFI 降级策略；更新 Rust 引擎模块
